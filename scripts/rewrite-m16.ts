...
const stateInsert = `
  ...
  const [cardTendered, setCardTendered] = useState<string>('');
  const [transferTendered, setTransferTendered] = useState<string>('');
...
`;
// Replace the top of state section:
content = content.replace("const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'CARD' | 'TRANSFER' | 'QR' | 'MIXED'>('CASH');", 
  "const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'CARD' | 'TRANSFER' | 'QR' | 'MIXED'>('CASH');" + stateInsert);

// 2. FIX HANDLE CONFIRM PAYMENT TO USE ASYNC AND OFFLINE QUEUE
const oldConfirmPaymentStr = content.substring(
  content.indexOf("const handleConfirmPayment = () => {"),
  content.indexOf("// Update shift totals")
);
...
const newConfirmPaymentStr = `
  const handleConfirmPayment = async () => {
    ...
    setShiftData(prev => ({
      ...prev,
      cashSales: prev.cashSales + Math.min(grandTotal, cashAmt - changeDue), // Only actual cash part minus change
  ...
  // Dummy to replace up to original "// Update shift totals"
  //`;

content = content.replace(oldConfirmPaymentStr, newConfirmPaymentStr);

// Cleanup the leftover Update shift totals
const leftoverShiftUpdate = \`
...
content = content.replace(leftoverShiftUpdate, "");
...
// We also need to fix the UI rendering for the promo engine in the cart mapping
...
// Add button to trigger Z-Report Modal
const zReportButton = \`
                <button
...
// Add Z-Report Modal JSX at the end before final div
...
\`;
content = content.replace(/<div className="grid grid-cols-3 gap-3">[\s\S]*?\.map\(\(m\) => \(/, paymentMethodRadios);
...
\`;
content = content.replace(/\{\/\* Cash Tendered Input \*\/\}[\s\S]*?\{\/\* End Cash Tendered \*\/\}/, mixedPaymentInputs);
content = content.replace(/\{paymentMethod === 'CASH' && \([\s\S]*?<\/div>\s*\)\}/, ""); // Remove the old block if regex failed
