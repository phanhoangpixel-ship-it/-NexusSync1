      ...
      <div className="px-5 py-3.5 bg-slate-900 text-white flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold px-3 py-1.5 rounded-xl border border-slate-700 outline-none cursor-pointer transition-colors"
          >
        ...
        <div className="p-5 flex flex-col gap-4 bg-slate-50/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
              ...
              <p className="text-xs text-slate-700 font-medium mt-1 truncate" title={activeJourney.nextStepRequirement}>
                  ...
                  <React.Fragment key={step.code}>
                    <div
                      ...
                      className={`flex-1 p-3 rounded-xl border transition-all cursor-pointer select-none flex flex-col justify-between ${
                        isSelected
                          ? 'ring-2 ring-blue-500 bg-blue-50/60 border-blue-300'
                          : isCurrent
          ...
          {selectedStepDetail && (
            <div className="p-4 rounded-xl bg-white border border-blue-200 shadow-2xs flex flex-col gap-2.5 animate-in fade-in duration-150">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-sm font-bold text-slate-900">{selectedStepDetail.name}</h4>
                </div>
                ...
                {onNavigateToStepModule && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold rounded-lg transition-colors cursor-pointer border border-blue-200"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-slate-600 mt-1">
                <div>
                  <span className="font-bold text-slate-700 block mb-0.5">Điều kiện tiên quyết:</span>
                ...
                <div>
                  <span className="font-bold text-slate-700 block mb-0.5">Kết quả đầu ra (Output):</span>
                  <p className="text-slate-600">{selectedStepDetail.output}</p>
                </div>

                <div>
                  <span className="font-bold text-slate-700 block mb-0.5">Phân quyền thực hiện:</span>
                  <div className="flex flex-wrap gap-1">
                    {selectedStepDetail.allowedRoles.map((r) => (
                      <span key={r} className="px-1.5 py-0.5 rounded bg-slate-100 font-mono text-[10px] text-slate-700 font-semibold">
                        {r}
                      </span>
                    ))}
                  </div>
