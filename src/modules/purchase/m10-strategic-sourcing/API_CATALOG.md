...
#### Error Response (422 Unprocessable Entity - Ineligible Supplier):
```json
{
  "success": false,
  ...
  "supplierId": 4,
  "supplierName": "Nhà cung cấp Vi phạm Chất lượng",
  "message": "Không thể tạo RFQ mời nhà cung cấp [Nhà cung cấp Vi phạm Chất lượng]: Nhà cung cấp đang trong danh sách tạm đình chỉ giao dịch (M09 SRM Guard)."
}
```
...
#### Error Response (422 Unprocessable Entity - Ceiling Price Exceeded):
```json
{
  "error": "CEILING_PRICE_EXCEEDED",
  "message": "Giá chào 370.000.000 VND vượt quá giá trần cho phép 350.000.000 VND của Vòng 2."
}
```
...
### 2.6. Chấm Điểm Đồng Thuận Ma Trận Đa Tiêu Chí (Consensus Evaluation)
* **Endpoint:** `POST /api/sourcing/rfqs/:id/consensus-evaluation`
* **RBAC Permissions:** `purchase:write`, `sourcing.evaluation.manage`
* **Description:** Tự động tính toán điểm số tổng hợp dựa trên 4 trụ cột nghiệp vụ:
  - **Thương mại / Giá chào (40%):** Nghịch đảo tỷ lệ giá so với giá chào thấp nhất thị trường.
  - **Kỹ thuật / Chất lượng (30%):** Thừa hưởng chỉ số Quality Rating từ thẻ điểm M11 SRM.
  - **Cam kết tiến độ SLA / OTIF (20%):** Kết hợp tỷ lệ giao hàng đúng hạn M11 (70%) và thời gian giao hàng (30%).
  - **Tuân thủ pháp lý & Bảo hành (10%):** Chỉ số tuân thủ hợp đồng M11.

---

### 2.7. Phê Duyệt Trao Thầu & Rào Chắn Ngân Sách M30 (Award Tender Decision)
* **Endpoint:** `POST /api/sourcing/awards`
* **RBAC Permissions:** `purchase:approve`, `sourcing.award.approve`
* **Description:** Phê duyệt quyết định trao thầu. Rào chắn ngân sách M30 Budget Guard kiểm tra trực tiếp tính khả dụng của Cost Center.

#### Error Response (422 Unprocessable Entity - Budget Exceeded):
```json
{
  "success": false,
  "error": "BUDGET_GUARD_EXCEEDED",
  "costCenter": "CC-RD",
  "requestedAmount": 680000000,
  "availableBudget": 40000000,
  "variance": -640000000,
  "message": "Không thể phê duyệt Quyết định Trao thầu: Tổng giá trị trúng thầu (680.000.000 ₫) vượt quá hạn mức ngân sách khả dụng của Cost Center [CC-RD] (40.000.000 ₫) tại Kỳ hạch toán hiện hành."
}
```

---

### 2.8. Ủy Quyền Khởi Tạo Đơn Mua Hàng M08 (PO Delegation Gateway)
* **Endpoint:** `POST /api/sourcing/awards/:id/generate-po`
* **RBAC Permissions:** `purchase:write`, `sourcing.po.delegate`
* **Description:** Thực hiện chuyển đổi từ Quyết định trúng thầu đã duyệt sang Đơn mua hàng chính thức trong M08. Tuân thủ **Single-Writer Authority**: Module M10 ủy quyền qua endpoint M08 `/api/purchase/orders`, tuyệt đối không ghi trực tiếp vào bảng `purchase_orders`.
...
```json
{
  ...
  "poId": 9,
  "poCode": "PO-2026-0009",
  "awardNo": "AWARD-2026-7358",
  "supplierId": 1,
  "totalAmount": 340000000,
  "message": "Đã ủy quyền khởi tạo thành công Đơn mua hàng PO-2026-0009 sang Phân hệ M08."
}
```

---

### 2.9. Niêm Phong Số Hồ Sơ Thầu M29 Secure Vault (SHA-256 Digital Seal)
* **Endpoint:** `POST /api/sourcing/awards/:id/seal-dms`
* **RBAC Permissions:** `purchase:write`, `dms:write`
* **Description:** Đóng băng và niêm phong toàn bộ hồ sơ trúng thầu, tính toán mã băm mật mã SHA-256 và lưu trữ vào kho hồ sơ bảo mật M29 DMS Vault với phân hạng lưu trữ bảo mật cao `HOT_ENCRYPTED`.
