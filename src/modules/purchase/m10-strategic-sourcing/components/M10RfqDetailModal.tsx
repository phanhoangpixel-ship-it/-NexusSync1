...
import { X, ShieldCheck, Calendar, DollarSign, Tag, Layers, UserPlus, Building, RefreshCw, CheckCircle2, Zap } from 'lucide-react';
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.message || errData.error || 'Lỗi mời nhà cung cấp');
      }

      onNotify('success', 'Mời Thầu Thành Công', 'Đã thẩm định điều kiện M09 và thêm nhà cung cấp vào danh sách nhận hồ sơ mời thầu.');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded text-[10px] font-mono tabular-nums font-bold bg-amber-900/80 text-amber-200 border border-amber-600 flex items-center gap-1">
                  <Zap className="w-3 h-3 text-amber-300" />
                  Vòng {rfq.currentRound || 1}
                </span>
              </div>
        ...
        <div className="p-5 space-y-4 text-xs overflow-y-auto grow">
          ...
          {/* Reverse Auction Quick Bar */}
          {onOpenReverseAuction && (
            <div className="p-3.5 rounded-xl bg-gradient-to-r from-purple-50 to-amber-50 dark:from-purple-950/40 dark:to-amber-950/40 border border-purple-200 dark:border-purple-800 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-amber-100 dark:bg-amber-950/70 border border-amber-300 dark:border-amber-700 flex items-center justify-center text-amber-700 dark:text-amber-300">
                  <Zap className="w-4 h-4" />
                </div>
                <div>
                  <span className="text-xs font-bold text-slate-900 dark:text-white block">
                    Đấu Thầu Ngược Đa Vòng (Vòng {rfq.currentRound || 1})
                  </span>
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">
                    Đàm phán giá giảm dần, thiết lập giá trần &amp; phân tích quỹ đạo tiết kiệm
                  </span>
                </div>
              </div>
              <button
                type="button"
                onClick={() => onOpenReverseAuction(rfq)}
                className="px-3 py-1.5 bg-purple-700 hover:bg-purple-800 text-white rounded-lg text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 shrink-0 cursor-pointer"
              >
                <Zap className="w-3.5 h-3.5" />
                <span>Mở Bảng Đấu Thầu Ngược</span>
              </button>
            </div>
          )}
          ...
          <div className="p-4 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-700 space-y-3">
            ...
            <form onSubmit={handleInvite} className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                >
                  <option value="">-- Chọn Nhà cung cấp để mời thầu (M09 Eligibility Guard) --</option>
                  {suppliers.map(s => {
                    const isEligible = s.isEligible !== false;
                    return (
                      <option 
                        key={s.id} 
                        value={s.id}
                        disabled={!isEligible}
                        className={isEligible ? '' : 'text-rose-500 bg-rose-50 dark:bg-rose-950/40'}
                      >
                        {isEligible ? '✓ ' : '⛔ '}
                        {s.name} ({s.code})
                        {!isEligible ? ` — [${s.ineligibilityReason || 'Không đủ điều kiện M09'}]` : (s.performanceTier ? ` — ${s.performanceTier}` : '')}
                      </option>
                    );
                  })}
                </select>
              ...
              {selectedSupplierToInvite && (() => {
                const targetSup = suppliers.find(s => String(s.id) === String(selectedSupplierToInvite));
                if (!targetSup) return null;
                const isEligible = targetSup.isEligible !== false;
                return (
                  <div className={`p-2 rounded-lg text-xs border flex items-center justify-between ${
                    isEligible 
                      ? 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300' 
                      : 'bg-rose-50 dark:bg-rose-950/40 border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-300'
                  }`}>
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold">M09 Guard:</span>
                      <span>{isEligible ? 'Hồ sơ đạt chuẩn thẩm định & đang hoạt động' : (targetSup.ineligibilityReason || 'Nhà cung cấp không đủ điều kiện dự thầu')}</span>
                    </div>
                    <span className="font-mono text-[11px] font-bold px-1.5 py-0.5 rounded bg-white/60 dark:bg-black/30">
                      {targetSup.status || 'ACTIVE'}
                    </span>
                  </div>
                );
              })()}
